...
HOURLY_COST = 1
# reward_episode file
FILE1 = "E:/training/weeklyR3/income_epi.txt"
# result file
FILE2 = "E:/training/weeklyR3/record/record_ep_{}.csv"

class weeklyR3Env(gym.Env):
    def __init__(self, data):
        self.df = data
        self.start_index = -168
        self.ep_number = 0
        file = open(FILE1, "a+")
        file.write("ep,income\n")
        file.close()
    ...
    def _take_action(self, action):
        ...
        self.yp = self.df.loc[self.current_index, 'norm_yp']
        if action == 0:
            # no operation
            self.percent = 0
        ...
        if 1 <= action <= 2:
            ...
            self.reward = self.e_sell * self.yp
        elif 3 <= action <= 4:
    ...
    def step(self, action):
        ...
        price = self.df.loc[self.current_index, 'yes_price']
        self._take_action(action)
        self.income = self.e_sell * price
        self.cumulative_income += self.income
        self.nb_steps += 1
    ...
    def render(self, mode='human', ):
        ...
        value1 = {
                 ...
                 "income(t)": self.income,
                  }
        self.record_per_ep = self.record_per_ep.append(value1, ignore_index=True)
        if self.done:
            file = open(FILE1, "a+")
            file.write(str(self.ep_number) + "," + str(self.cumulative_income) + "\n")
            file.close()
            if self.ep_number % 10 == 0:
                self.record_per_ep.to_csv(FILE2.format(self.ep_number))
